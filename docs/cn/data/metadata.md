&emsp;&emsp;GNSS元数据指台站的相关信息，包括台站所有权、联系方式、标石信息、台站坐标及设备安装历史。可靠且最新的元数据对GNSS测站管理与使用至关重要，由测站操作员负责维护。然而，在确有必要（例如更正不准确的源数据、更新台站日志格式等）且测站运营方对所建议的变更未作回应的情况下，IGS 网络协调员保留更新测站元数据的权利。
